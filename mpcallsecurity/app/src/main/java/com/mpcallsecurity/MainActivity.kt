...
class MainActivity : Activity() {
    ...
    private fun refresh(){val active=isActive();status.text=if(active)"● FILTRAGEM ATIVA" else "○ FILTRAGEM INATIVA";val available=Build.VERSION.SDK_INT>=29&&getSystemService(RoleManager::class.java).isRoleAvailable(RoleManager.ROLE_CALL_SCREENING);diagnostic.text="Status do Android: função disponível=$available • função atribuída=$active";stats.text="Bloqueios: ${CallRules.logs(this).size} • Lista negra: ${CallRules.list(this,"blocked_numbers").size} • Prefixos: ${CallRules.list(this,"blocked_prefixes").size}";logsText.text=CallRules.logs(this).ifEmpty{"Nenhuma chamada bloqueada ainda."}.joinToString("\n")}
}
