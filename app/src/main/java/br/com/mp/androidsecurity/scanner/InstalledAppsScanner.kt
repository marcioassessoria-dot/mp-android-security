...
class InstalledAppsScanner(private val c:Context){
 ...
 fun scan(acc:Set<String>,admins:Set<String>):List<InstalledAppInfo>=pm.getInstalledPackages(PackageManager.GET_PERMISSIONS).mapNotNull{pkg->
  ...
  val requested=pkg.requestedPermissions.orEmpty().filter(RiskEngine::isSensitive)
  ...
  InstalledAppInfo(pkg.packageName,ai.loadLabel(pm).toString(),pkg.versionName,versionCode,ai.targetSdkVersion,(ai.flags and ApplicationInfo.FLAG_SYSTEM)!=0,ai.enabled,installer,pkg.firstInstallTime,pkg.lastUpdateTime,findings,accessibility,admin,RiskEngine.reasons(requested,granted,accessibility,admin),score,RiskEngine.level(score))
