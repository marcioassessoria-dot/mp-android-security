...
class AdBlockVpnService:VpnService(){
 ...
 private fun udpChecksum(p:ByteArray,ihl:Int):Int{var s=0L;for(i in 12 until 20 step 2)s+=u16(p,i);s+=17L;s+=(p.size-ihl-8).toLong();s+=onesComplementSum(p,ihl,p.size-ihl);while(s ushr 16!=0)s=(s and 65535)+(s ushr 16);val v=s.inv().toInt() and 65535;return if(v==0)65535 else v}
 private fun onesComplementSum(p:ByteArray,start:Int,len:Int):Long{var s=0L;var i=start;val end=start+len;while(i+1<end){s+=u16(p,i);i+=2};if(i<end)s+=((p[i].toInt() and 255) shl 8).toLong();while(s ushr 16!=0)s=(s and 65535)+(s ushr 16);return s}
