...
class AdBlockVpnService:VpnService(){
 ...
 private fun resolveUid(src:ByteArray,sp:Int,dst:ByteArray,dp:Int):Int{if(Build.VERSION.SDK_INT<29)return -1;return runCatching{getSystemService(android.net.ConnectivityManager::class.java).getConnectionOwnerUid(OsConstants.IPPROTO_UDP,InetSocketAddress(InetAddress.getByAddress(src),sp),InetSocketAddress(InetAddress.getByAddress(dst),dp))}.getOrDefault(-1)}
