...
data class ScanResult(val apps:List<InstalledAppInfo>,val browsers:List<InstalledAppInfo>,val accessibility:List<AccessibilityFinding>,val deviceAdmins:List<DeviceAdminFinding>,val monitoring:MonitoringStatus,val privateDns:PrivateDnsStatus,val diagnostics:DeviceDiagnostics,val durationMs:Long){
 val highRiskCount get()=apps.count{it.riskScore>=80}; val suspiciousCount get()=apps.count{it.riskLevel==RiskLevel.SUSPICIOUS}; val attentionCount get()=apps.count{it.riskLevel==RiskLevel.ATTENTION}; val lowRiskCount get()=apps.count{it.riskLevel==RiskLevel.LOW}; val possibleAdwareCount get()=apps.count{it.adwareLevel!=AdwareLevel.NONE}; val highAdwareCount get()=apps.count{it.adwareLevel==AdwareLevel.HIGH}
}