...
data class ScanResult(val apps:List<InstalledAppInfo>,val accessibility:List<AccessibilityFinding>,val deviceAdmins:List<DeviceAdminFinding>,val durationMs:Long){val highRiskCount get()=apps.count{it.riskLevel==RiskLevel.HIGH};val suspiciousCount get()=apps.count{it.riskLevel==RiskLevel.SUSPICIOUS};val attentionCount get()=apps.count{it.riskLevel==RiskLevel.ATTENTION};val lowRiskCount get()=apps.count{it.riskLevel==RiskLevel.LOW}}